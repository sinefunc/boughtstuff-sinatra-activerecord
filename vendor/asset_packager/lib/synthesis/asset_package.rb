module Synthesis
  class AssetPackage
    ...
    @@asset_packages_yml = $asset_packages_yml || 
      (File.exists?("#{ROOT_DIR}/config/asset_packages.yml") ? YAML.load_file("#{ROOT_DIR}/config/asset_packages.yml") : nil)
    ...
    class << self
      ...
      def create_yml
        unless File.exists?("#{ROOT_DIR}/config/asset_packages.yml")
          asset_yml = Hash.new

          asset_yml['javascripts'] = [{"base" => build_file_list("#{ROOT_DIR}/public/javascripts", "js")}]
          asset_yml['stylesheets'] = [{"base" => build_file_list("#{ROOT_DIR}/public/stylesheets", "css")}]

          File.open("#{ROOT_DIR}/config/asset_packages.yml", "w") do |out|
    ...
    def initialize(asset_type, package_hash)
      ...
      @asset_path = ($asset_base_path ? "#{$asset_base_path}/" : "#{ROOT_DIR}/public/") +
    ...
    private
      ...
      def compress_js(source)
        jsmin_path = "#{ROOT_DIR}/bin"
        tmp_path = "#{ROOT_DIR}/tmp/#{@target}_packaged"
